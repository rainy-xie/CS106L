...
void parse_csv(std::string filename, std::vector<Course> &vector_of_courses) {
  ...
  while (std::getline(ifs, line)) {
    ...
    vector_of_courses.emplace_back(
        Course{raw_data[0], raw_data[1], raw_data[2]});
    // Course new_course = {raw_data[0], raw_data[1], raw_data[2]};
    // vector_of_courses.push_back(new_crouse);
  }
}
